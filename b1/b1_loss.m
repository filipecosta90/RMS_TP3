...
l = legend('full loss','flow 0 -> 8 bw ( 3Mb CBR App ) :: Client 1 to Client 6', 'flow 1 -> 6 bw ( FTP connection ) :: Client 2 to Client 4',  'flow 1 -> 7 bw ( FTP connection ) :: Client 2 to Client 5' , 'flow 2 -> 6 bw ( VOIP connection over UDP ) :: Client 3 to Client 4', 'flow 6 -> 2 bw ( VOIP connection over UDP ) :: Client 4 to Client 3',  'flow 7 -> 1 bw ( FTP connection ) :: Client 5 to Client 2', 'flow 8 -> 0 bw ( 3Mb CBR App ) :: Client 6 to Client 1');
