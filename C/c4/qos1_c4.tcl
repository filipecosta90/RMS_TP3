...
set trace_file [open qos1_c4.tr w]
...
$q(E1C0) addPolicyEntry -1 -1 Dumb 10       ;# todo o restante trafego...
#$q(E1C0) addPolicerEntry TokenBucket 10 11
$q(E1C0) addPolicerEntry Dumb 10
...
# The addPolicyEntry commands establish two policies at the edge queue: 
#   --- one between nodes $Cli6 and $Cli1 -> TokenBucket
#   and 
#   --- one between every any initial and end station -> Dumb(does nothing).
# The CIR and CBS values used in the policies are the ones set at the beginning of the script.
#$q(E2C0) addPolicyEntry [$Cli6 id] [$Cli1 id] TokenBucket 10 $cir0 $cbs0
$q(E2C0) addPolicyEntry -1 -1 Dumb 10

# The following command adds an entry to the Policer Table, specifying that the TokenBucket has initial (green) code point 10,
# downgraded (yellow) code point 11:
#$q(E2C0) addPolicerEntry TokenBucket 10 11

# The following command adds an entry to the Policer Table, specifying that the Dumb has initial (yellow) code point 11 
# >>> Every traffic is marked the same (yellow):
$q(E2C0) addPolicerEntry Dumb 10
